...
import Style from "./productRoute.module.scss";
import { fetchProductRoute } from "../redux/Slice";
...
const ProductRoute = () => {
  ...
  useEffect(() => {
    dispatch(fetchProductRoute());
  }, [dispatch]);
  console.log(`/product/product/${userId}`);

  const filterData =
    data.productRoute &&
    data.productRoute.filter((item) => item.url === `product/${userId}`);
